...
resource "aws_security_group_rule" "https_access_cidr"{
  count             = var.enable_https && (local.cidr_block_ingress_rule > 0 ? true : false ) ? 1 : 0
...
resource "aws_security_group_rule" "https_access_sg"{
  count = var.enable_https && (local.security_group_ingress_rule > 0 ? true : false ) ? 1 : 0
...
resource "aws_security_group_rule" "http_access_cidr"{
  count             = var.enable_http && (local.cidr_block_ingress_rule > 0 ? true : false ) ? 1 : 0
...
resource "aws_security_group_rule" "http_access_sg"{
  count = var.enable_http && (local.security_group_ingress_rule > 0 ? true : false ) ? 1 : 0
